...
class ChatControllerTest {
    ...
    @DisplayName("Verify getAll() method works for ChatController")
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    @Test
    void getAll_ValidRequest_ReturnListChatDto() throws Exception {
        List<ChatDto> expected = testAllChats();
        ...
        ChatDto[] actual = objectMapper.readValue(
                result.getResponse().getContentAsByteArray(), ChatDto[].class
        );
        assertEquals(expected, Arrays.stream(actual).toList());
    }

    @DisplayName("Verify getById() method works for ChatController")
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    @Test
    void getById_ValidRequest_ReturnChatDto() throws Exception {
        ChatDto expected = testChat();
        MvcResult result = mockMvc.perform(
                        get("/chats/1")
                                .contentType(MediaType.APPLICATION_JSON.getMediaType())
                )
    ...
    @DisplayName("Verify getLog() method works for ChatController")
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    @Test
    ...
    private List<ChatDto> testAllChats() {
        return List.of(
                new ChatDto(
                        1L,
                        123456789L,
